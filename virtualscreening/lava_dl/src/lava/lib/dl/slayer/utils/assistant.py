            ...
            if self.stats is not None:
                ...
                if self.classifier is not None:   # classification
                    self.stats.testing.correct_samples += torch.sum(
                        self.classifier.predict(output) == target
                    ).cpu().data.item()
                    self.stats.testing.test_conf.append(self.classifier.confidence(output,mode="softmax"))
                    self.stats.testing.test_pred.append(self.classifier.predict(output))
                    self.stats.testing.test_labels.append(target)
